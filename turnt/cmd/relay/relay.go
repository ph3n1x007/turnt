...
import (
	...
	turntproxy "github.com/praetorian-inc/turnt/internal/proxy"
...
func main() {
	...
	proxyFlag := flag.String("proxy", "", "HTTP proxy URL (e.g. http://proxy:8080) or 'auto' to detect from system/environment")
	flag.Parse()
	...
	if *offerFlag == "" {
		...
		fmt.Println("Usage: ./relay -offer \"<Base64_Offer>\" [-proxy auto|<url>] [-log-file <path>] [-offer-file <path>] [-verbose]")
		return
	}
	...
	if *proxyFlag != "" {
		proxyURL := *proxyFlag
		if proxyURL == "auto" {
			proxyURL = turntproxy.DetectProxy()
			if proxyURL == "" {
				fmt.Println("[i] No proxy detected from system/environment")
			} else {
				fmt.Printf("[+] Auto-detected proxy: %s\n", proxyURL)
			}
		}
		if proxyURL != "" && proxyURL != "auto" {
			dialer, err := turntproxy.NewHTTPConnectDialer(proxyURL)
			if err != nil {
				fmt.Printf("[-] Error configuring proxy: %v\n", err)
				return
			}
			proxyDialer = dialer
	...
	peerConn, err := webrtc.NewPeerConnection(offerPayload.ICEServers, proxyDialer)
	if err != nil {
